...
def _sigma_delta(particles: xt.Particles) -> float:
    ...
    delta_np = nplike.to_numpy(delta)

    core_delta = extract_core_from_gradient(delta_np, threshold_frac=0.01)

    if len(core_delta) < 2:
        # Fallback to using all alive particles
        return float(nplike.std(delta))

    return float(np.std(core_delta))
